...
private const val TAG = "HackatimeApi[listUserProjectDetails]"

suspend fun listUserProjectDetails(
    context: Context,
    username: String,
    projects: List<String>? = null,
    projectsStartDate: String? = null,
    projectsEndDate: String? = null,
    statsStartDate: String? = null,
    statsEndDate: String? = null
): Result<List<ProjectDetails>> {
    try {
        val response = ApiClient.hackatimeApi.listUserProjectDetails(
            username = username,
        ...
        if (body is ListUserProjectDetailsResponse) {
